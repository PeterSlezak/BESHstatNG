...
- [Mixed Models for Repeated Measures (MMRM)](methods/mixed-models-for-repeated-measures-mmrm.md) — *Gaussian MMRM, ML / REML, subject-level residual covariance structures, Satterthwaite and Kenward-Roger inference, LS-means, contrasts, change from baseline, reference-grid estimated marginal means, diagnostics, and worksheet UDFs.* Fit a marginal repeated-measures model for continuous longitudinal outcomes and estimate visit-specific means and treatment differences.
- [Linear Mixed Models (LMM)](methods/linear-mixed-models-lmm.md) — *Gaussian linear mixed models, ML / REML, fixed effects with categorical factors, polynomial terms and interactions, random intercepts, random slopes and random-effect interactions, G-side covariance structures (RI, RI+slope, ID, VC/Diag, CS, CSH, AR1, ARH1, TOEP, TOEPH, Unstructured), optional R-side residual covariance structures, Satterthwaite and Kenward-Roger inference, fitted values/residuals, BLUPs, covariance/correlation output, diagnostics and trace output.* Fit subject- or cluster-level random-effects models for continuous outcomes, including random-intercept, random-slope and richer mixed-model specifications.
- [Cox Regression](methods/cox-regression.md) — *Cox proportional hazards model • Tie handling: Breslow, Efron, Exact • Robust variance (optional) • Residuals + PH score test (optional) • Baseline + adjusted survival curves.* Fit a proportional hazards model to time-to-event data and report hazard ratios with diagnostics.
...
- [Intraclass Correlation Coefficients](methods/intraclass-correlation-coefficients.md) — *Propensity score estimation by GLM logistic regression or supplied scores; nearest-neighbor, optimal pair, subclassification, weighting, coarsened exact matching, exact matching restrictions, calipers, common support and trimming; ATT / ATC / ATE / ATO estimands; balance diagnostics, Love-plot data, overlap and weight diagnostics, AIPW doubly robust estimates, bootstrap summaries, and Rosenbaum-style matched-pair sensitivity analysis.* Estimate treatment effects for observational studies by balancing treated and control groups using propensity-score-based matching, weighting, and diagnostics.
