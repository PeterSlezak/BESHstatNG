...
### Regression
- [Multiple Linear Regression (LM)](methods/multiple-linear-regression-lm.md) — *OLS multiple regression • Optional weights • Type I or Type III term SS (ANOVA table) • Optional covariance matrix and residuals.* Fit a standard linear regression model, report coefficients and ANOVA-style term tests.
...
- [Deming Regression](methods/deming-regression.md) — *Deming (errors-in-variables) regression with configurable error ratio (λ), point estimates and confidence intervals.* Use when both X and Y have measurement error and you want a symmetric method-comparison regression.
