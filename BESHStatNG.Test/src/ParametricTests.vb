...
Public Class Parametric_Tests
    ...
    Public Sub OneWayANOVA_compute_matches_reference()
        ...
        Dim a As New parametric.OneWayANOVA(x, names)
        Dim tbl = a.compute()
    ...
    Public Sub OneWayANOVA_FisherLSD_matches_reference()
        ...
        For i = 0 To 2
            For j = i + 1 To 2
                ...
                Dim se As Double = Math.Sqrt(MSerr * (1.0 / n(i) + 1.0 / n(j)))
                Dim t As Double = diff / se
                Dim p As Double = distributions.T_2T(Math.Abs(t), DFerr)
    ...
    Public Sub OneWayANOVA_GamesHowell_matches_reference()
        ...
        For k = 0 To comps.Count - 1
            ...
            Dim adiff As Single, alcl As Single, aucl As Single
            ParseDiffCi(CStr(got(k, 1)), adiff, alcl, aucl)
            Assert.AreEqual(CSng(comps(k).diff), adiff, TOL_SNG, "Diff mismatch (GamesHowell)")
    ...
    Public Sub OneWayRmANOVA_compute_matches_reference()
        ...
        Dim a As New parametric.OneWayRmANOVA(x, names)
        Dim tbl = a.compute()
    ...
    Public Sub OneWayRmANOVA_Tukey_matches_reference()
        ...
        Dim MSerr As Double = CDbl(ANOVAtable(2, 2))
        ...
        For k = 0 To comps.Count - 1
            ...
            Assert.AreEqual(CSng(comps(k).lcl), alcl, 0.000001, "LCI mismatch (RM Tukey)")
            Assert.AreEqual(CSng(comps(k).ucl), aucl, 0.000001, "UCI mismatch (RM Tukey)")
    ...
    Public Sub UnpairedTtest_compute_matches_reference()
        ...
        Dim t As New parametric.UnpairedTtest(x, names)
        Dim res = t.compute()
    ...
    Public Sub PairedTtest_compute_matches_reference()
        ...
        Dim t As New parametric.PairedTtest(x, names)
        Dim res = t.compute()
    ...
    Public Sub HotelingsT_independent_equal_cov_matches_reference()
        ...
        Dim ht As New parametric.HotelingsT_independent(x1, x2, names)
    ...
    Public Sub HotelingsT_independent_CI_matches_reference()
        ...
        Dim tcrit As Double = Math.Sqrt(distributions.F_Inv(1 - 0.05, p, df2) * p * (n1 + n2 - 2) / df2)
    ...
    Public Sub HotelingsT_single_matches_reference()
        ...
        Dim ht As New parametric.HotelingsT_single(x, H0, names)
    ...
    Public Sub HotelingsT_single_CI_matches_reference()
        ...
        Dim tcrit As Double = Math.Sqrt(p * (n - 1) / (n - p) * distributions.F_Inv_RT(0.05, p, n - p))
    ...
    Public Sub HotelingsT_paired_matches_reference()
        ...
        Dim ht As New parametric.HotelingsT_paired(x1, x2, names)
        Dim res = ht.calculate()
