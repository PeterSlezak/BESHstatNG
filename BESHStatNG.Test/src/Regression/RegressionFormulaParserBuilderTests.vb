...
Public Class RegressionFormulaParserBuilderTests
    ...
    <TestMethod>
    <TestCategory("RegressionFormula")>
    Public Sub FormulaParser_ValidatesFactorByContinuousInteraction()
        Dim x(17, 1) As Double
        For i As Integer = 0 To 17
            x(i, 0) = If(i Mod 2 = 0, 0.0, 1.0)
            x(i, 1) = CDbl(i + 1)
        Next
        ...
        Dim ok As Boolean = RegressionFormulaDesignService.TryBuildExpandedPredictorMatrixFromFormula(
            ...
            result:=result,
            errorMessage:=err,
            predictorNames:=New String() {"A", "B"},
            formulaText:="factor(A) + B + factor(A):B",
        ...
        Assert.AreEqual(3, result.ExpandedPredictorMatrix.GetLength(1))
    End Sub

    <TestMethod>
    <TestCategory("RegressionFormula")>
    Public Sub FormulaParser_ValidatesFactorByFactorInteraction()
        Dim x(17, 1) As Double
        For i As Integer = 0 To 17
            x(i, 0) = CDbl(i Mod 2)
            x(i, 1) = CDbl((i \ 3) Mod 3)
        Next
        ...
        Dim ok As Boolean = RegressionFormulaDesignService.TryBuildExpandedPredictorMatrixFromFormula(
            ...
            errorMessage:=err,
            predictorNames:=New String() {"A", "B"},
            formulaText:="factor(A) + factor(B) + factor(A):factor(B)",
            allowRelativeColumnLetters:=True,
            allowAbsoluteColumnLetters:=False,
            allowQuotedVariableNames:=True,
            omitCategoricalReference:=True)
        ...
        Assert.IsTrue(ok, err)
        Assert.IsNotNull(result)
        ' A has 2 levels -> 1 dummy; B has 3 levels -> 2 dummies; interaction -> 1*2 = 2 columns.
