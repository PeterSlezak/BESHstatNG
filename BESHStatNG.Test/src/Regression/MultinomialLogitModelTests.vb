...
Public Class MultinomialLogitModel_Tests
    ...
    Private Shared Function ExpandByIntegerWeights(data(,) As Double, offs() As Double, w() As Double) As Tuple(Of Double(,), Double())
        ...
        For i As Integer = 0 To n - 1
            ...
            If Math.Abs(wi - wiInt) > 0.0000001 Then
                Throw New InvalidOperationException("Weights must be (near) integers for expansion test. RowUdfs " & i & " has w=" & wi.ToString(CultureInfo.InvariantCulture))
            End If
            total += wiInt
        Next
    ...
    Public Sub Fit_grouped_basic_matches_reference_outputs()
        ...
        m.Fit(intercept:=1, reference:=ReferenceCategory.Last)
    ...
    Public Sub ReferenceCategory_First_and_Last_have_identical_probabilities_and_LL()
        ...
        mLast.Fit(intercept:=1, reference:=ReferenceCategory.Last)
        ...
        mFirst.Fit(intercept:=1, reference:=ReferenceCategory.First)
    ...
    Public Sub Grouped_weights_equivalent_to_expanded_unweighted_fit()
        ...
        mW.Fit(intercept:=1, reference:=ReferenceCategory.Last)
        ...
        mExp.Fit(intercept:=1, reference:=ReferenceCategory.Last)
    ...
    Public Sub Zero_and_negative_weights_are_ignored_like_row_removal_in_fit()
        ...
        mAll.Fit(intercept:=1, reference:=ReferenceCategory.Last)
        ...
        mDrop.Fit(intercept:=1, reference:=ReferenceCategory.Last)
    ...
    Public Sub Calculate_throws_when_no_predictors_and_intercept_zero()
        ...
        Dim m As New MultinomialLogitModel()
        m.data(data, names)
        Assert.ThrowsException(Of ArgumentException)(Sub() m.Fit(intercept:=0))
    End Sub
    ...
    Public Sub Calculate_throws_when_startParams_length_mismatch()
        ...
        Assert.ThrowsException(Of ArgumentException)(Sub() m.Fit(intercept:=1, reference:=ReferenceCategory.Last, bStartParams:=True))
    End Sub
